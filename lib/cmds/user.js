...
User.authorizationCallback_ = function(user, err, res, opt_callback) {
    logger.defaultCallback(err, null, 'Authentication succeed.');

    if (res.token) {
        base.writeGlobalConfigCredentials(user, res.token);
        ...
        [
            ...
            base.github.authorization.create(payload, function(err, res) {
                ...
                if (isTwoFactorAuthentication) {
                    ...
                    User.authorizationCallback_(answers.user, err, res, opt_callback);
                }
            });
        });
};
...
User.twoFactorAuthenticator_ = function(payload, user, opt_callback) {
    inquirer.prompt(
        [
            ...
            base.github.authorization.create(payload, function(err, res) {
                User.authorizationCallback_(user, err, res, opt_callback);
            });
        });
};
