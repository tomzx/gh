...
Repo.prototype.run = function() {
    ...
    if (options.fork) {
        hooks.invoke('repo.fork', instance, function(afterHooksCallback) {
            ...
            logger.logTemplate(
                '{{prefix}} [info] Forking repo {{greenBright options.user "/" options.repo}} on {{greenBright user "/" options.repo}}', {
                    options: options,
                    user: user
                });
...
Repo.prototype.clone_ = function(user, repo, repo_url) {
    logger.logTemplate(
        '{{prefix}} [info] Cloning {{greenBright user "/" repo}}', {
            user: user,
            repo: repo
        });

    git.clone(
        url.parse(repo_url).href,
        repo,
        function(err, data) {
            console.log(data);
        });
