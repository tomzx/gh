...
PullRequest.prototype.run = function() {
    ...
    if (options.list) {
        ...
        if (options.all) {
            logger.logTemplate(
                '{{prefix}} [info] Listing all {{options.state}} pull requests for {{greenBright options.user}}', {
                    options: options
                });
            ...
            if (options.me) {
                logger.logTemplate(
                    '{{prefix}} [info] Listing {{options.state}} pull requests sent by {{greenBright options.loggedUser}} on {{greenBright options.user "/" options.repo}}', {
                        options: options
                    });
            }
            else {
...
PullRequest.prototype.fetch = function(opt_type, opt_callback) {
    ...
    operations = [
        ...
        function(callback) {
            git.exec('fetch', [repoUrl, headBranch + ':' + options.pullBranch], function(status,
                err) {
                callback(err);
            });
        },
